...
impl Difference {
    ...
    fn get_usize_type_to_bytes(value: usize) -> Vec<u8> {
        if value <= 255 {
    ...
    fn get_usize_type_from_bytes(bytes: &[u8]) -> (usize, usize) {
        match bytes[0].into() {
            USizeType::U16 => {
                let mut buffer = [0; 2];
...
impl SimpleDirectDeltaEncoding {
    ...
    pub fn apply_diff(data: &mut [u8], diff: &[Difference]) {
        for d in diff {
            if d.action == DifferenceAction::Replace {
                ...
                data[d.range.start..d.range.start].copy_from_slice(&d.value);
            } else if d.action == DifferenceAction::Delete {
                data[d.range.start..(d.range.start + d.range.length)].copy_from_slice(&[]);
            }
        }
    }
}